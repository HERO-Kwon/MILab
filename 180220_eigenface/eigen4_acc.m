...
for i = 1:length(train_data)
    sigma_f_train{i} = zeros(height*width,1);
    for k = 1:n_eigface
        weight_train(k) = transpose(eig_face{k}) * sigma{i};
        sigma_f_train{i} = sigma_f_train{i} +  weight_train(k) * eig_face{k};
    end
...
for i = 1:length(test_data)
    err_class_acc = [];
    % Search Closest face
    for k = 1:length(train_data) %length(test_data)
        err_class_acc(k) = norm(weights_train{i} - weights_test{k},2);
    end
    
    [val_acc,ind_acc] = min(err_class_acc);
    
    pred_person = train_data(ind_acc).person;
    act_person = test_data(i).person;
...
acc_id = pred_tf / length(test_data) 