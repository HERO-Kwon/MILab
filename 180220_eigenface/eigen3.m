...
test_data_face = [test_data;simg_data];
...
err_sampled = datasample(err_allval,100);
fnfp = [];
...
for i = 1:length(test_data_face)
    err_face(i) = norm(sigma_new{i} - sigma_f{i});
    test_data_face(i).is_person = ( err_face(i) < thres_face );
end