...
person_group = findgroups(cellstr(char(train_data.person)));
...
for i = 1:length(test_data)
    ...
    err_underthres = ( err_class < thres_person );
    test_data(i).predicted_person = splitapply(@max,err_underthres',person_group);
end
...
for i = 1:length(test_data)
    ...
    person_array(str2num(test_data(i).person)) = 1;

    [g1,gid1] = findgroups(test_data(i).predicted_person);
    [g2,gid2] = findgroups(person_array);
