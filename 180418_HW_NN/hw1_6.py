...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            weights = {
                'w_h1' : tf.Variable(tf.random_normal([2,n_hidden_1])),
                'w_out' : tf.Variable(tf.random_normal([n_hidden_1,1]))
            }
            biases = {
                'b_h1' : tf.Variable(tf.zeros([1,n_hidden_1])),
...
result_hn1 = pd.DataFrame(columns = ['n_hidden','acc_test','acc_train'])
for iter in range(1,2):
    print('processing : ',iter)
    for j in range(0,1):
        result = run_model(iter)
        res_df = {'n_hidden':result[0],'acc_test':result[1],'acc_train':result[2]}
...
# Plot the surface.
surf = ax.plot_surface(y_mesh, x_mesh, f_target, cmap=cm.coolwarm,
