...
data_read_f =  data_read #np.array([row for row in list(data_read) if np.max(np.abs(row[4:])) <= xs_th])


label_data = data_read[:,0]
...
no_classes = len(np.unique(data_read[:,0]))

data_y.shape
idx_tr,idx_te = train_test_split(data_read_f,test_size=0.2,random_state=10)
data_read_f.shape

# train test split
idx_tr,idx_te = train_test_split(np.arange(len(data_read_f)),test_size=0.2,random_state=10)
#input data
data_xs_tr = data_read_f[idx_tr,4:].astype('float32') / xs_th
data_xs_te = data_read_f[idx_te,4:].astype('float32') / xs_th
...
x_train = data_xs_tr.reshape([-1,6,30,500]).swapaxes(1,3)
x_test = data_xs_te.reshape([-1,6,30,500]).swapaxes(1,3)
