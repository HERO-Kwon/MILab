...
def ReadData(PATH,n_splits):
    ...
    data_list = []
    for train_index,test_index in skf.split(arr_csi,arr_lab[:,0]):
    ...
    #idx_te = np.arange(len(arr_csi))[600:]
        ...
        X = arr_csi[train_index]
        Xval = arr_csi[test_index]
        c = {}
        cval = {}
        Y = arr_lab[train_index]
        Yval = arr_lab[test_index]
...
for splits in range(len(data_skf)):
    ...
    for i in range(n_classes):
        ...
        for ti in range(n_classes):
            ...
            score_list = [splits,i,ti,int(get_key(i,cval) == get_key(ti,cval)),score[0][0]]
...
#res_eer.to_csv('res_eer_loc1.csv')
