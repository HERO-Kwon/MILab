...
def TERmodel_prim(r,n,X,Y,mod_w):
...
def TERmodel_dual(r,n,X,Y,mod_w):
    #simplified version of TER
    beta = []
    for k in list(set(Y)):
        P = X
...
for splits in range(2): #range(len(data_skf)):
    ...
    W,u = Fe_HC(data_xs,pca_c,pca_s,40)
    
    pca_train = np.dot(W,data_xs.T-u).T
    pca_test = np.dot(W,data_xsv.T-u).T
    
    for ii in range(10):
        t1 =time.time()
        #alpha = TERmodel_new(0.5,0.5,data_xs,data_y,M_dict[str(ii)])
        beta = TERmodel_dual(0.5,0.5,data_xs,data_y,M_dict[str(ii)])
        
        #beta = LSEdual(data_xs,data_y)
        #data_test = data_xsv.dot(alpha)
        
        data_test = np.dot(data_xsv,data_xsv.T).dot(beta)
        ...
        for i in range(n_classes):
            for ti in range(n_classes):
                eer_dist = np.abs(data_test[i,:] - data_test[ti,:])
...
for s in range(2): #range(2):
    for ii in range(10):
        ...
        print(s,ii)
        med_score = np.median(res_eer.score)
        thres_range = np.arange(0,2*med_score,2*med_score / 100)
        res_eer_val,_ = EER_Curve(res_plot.TF,res_plot.score,thres_range)
