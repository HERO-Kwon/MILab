...
path_save = 'D:\\Data\\sig2d_processed1\\'
...
for file in img_list:
    ...
    for j in range(5):
        img1 = img[pnts1[j,1]+margin:pnts1[j+1,1]-margin,pnts1[j,0]+margin:pnts2[j,0]-margin]
        cv2.imwrite(path_save+img_name+'_'+str(j)+'.png',cv2.resize(img1,(625,375),1,1,interpolation=cv2.INTER_AREA))
        #plt.imshow(img1)
    for k in range(5):
        img1 = img[pnts2[k,1]+margin:pnts2[k+1,1]-margin,pnts2[k,0]+margin:pnts3[k,0]-margin]
        cv2.imwrite(path_save+img_name+'_'+str(k+5)+'.png',cv2.resize(img1,(625,375),1,1,interpolation=cv2.INTER_AREA))
        #plt.imshow(img1)