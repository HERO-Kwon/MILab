...
def ReadData(PATH,n_splits,rseed):
    ...
    for i in locs:
        for j in dirs:
            filename = 'csi_v2_csi_' + str(i) + '_' + str(j) + '.npy'
            file_list.append(filename)
    # Filter Dir
    
    for file in file_list:
        data_read = np.load(PATH + file)
        lab_read = np.load(PATH + 'csi_v2_lab_' + str(i) + '_' + str(j) + '.npy')
        csi_read = np.abs(data_read).astype('float32')
        #lab_read = data_read[:,0].astype('int')

        data_x = csi_read#.reshape([-1,10,6,30,500]).swapaxes(2,4)

        uniq_label = np.unique(lab_read[:,0])
        label_table = pd.Series(np.arange(len(uniq_label)),index=uniq_label)
        data_y = np.array([label_table[num] for num in lab_read[:,0]]).reshape([-1,10])
    ...
    arr_csi = np.array(list_csi).swapaxes(0,1).reshape([98*len(file_list),10,1500,30,6])
    arr_lab = np.array(list_lab).swapaxes(0,1).reshape([98*len(file_list),10])

    skf = StratifiedKFold(n_splits,random_state=rseed)
    data_list = []

    arr_csi1 = arr_csi.reshape([-1,1500,30,6])
    ...
    #idx_te = np.arange(len(arr_csi))[600:]
        ...
        X = arr_csi1[train_index].reshape([-1,1,1500,30,6])
        Xval = arr_csi1[test_index].reshape([-1,1,1500,30,6])
        c = {}
        cval = {}
