...
def load_images_from_folder(folder, n_im, normalize=False, imrotate=False):
    ...
    for filename in os.listdir(folder):
        if not filename.startswith('.'):
            ...
            bigy_temp = sio.loadmat(os.path.join(folder,filename))['im_org']
            bigy[im, :, :] = bigy_temp
...
tic1 = time.time()
dir_train = 'D:\Matlab_Drive\Data\MedAI\MAI_Project1\Train_image_label'  # Folder with images
n_im = 10  # How many images to load
...
model(np.float16(X_train), np.float16(Y_train),
