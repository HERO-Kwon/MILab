    ...
    def get_batch(self,batch_size,s):
        ...
        for i in range(batch_size):
            ...
            if(s=='train'):
                ...
                # pos
                kar_same = self.kar_fc[cat_same]
                dist_same = np.linalg.norm([kar_same - kar_ank],ord=2,axis=-1).squeeze()
                dist_hard_p = np.percentile(dist_same, 75, interpolation='nearest')
                hard_samples_p = dist_same >= dist_hard_p
                cat_diff_hard_p = list(np.array(cat_same)[hard_samples_p])
                
                category_s1 = rng.choice(cat_diff_hard_p,size=1,replace=False)[0]
                pairs[1][i,:,:,:] = X[category_s1, idx_1].reshape(w, h, ch)
                # neg
                ...
                category_d1 = rng.choice(cat_diff_hard,size=1,replace=False)[0]
                pairs[2][i,:,:,:] = X[category_d1, idx_1].reshape(w, h, ch)
            else:
                #category_d = rng.choice(cat_diff,size=1,replace=False)[0]
                pairs[1][i,:,:,:] = X[category_s, idx_1].reshape(w, h, ch)
                pairs[2][i,:,:,:] = X[category_d, idx_1].reshape(w, h, ch)
