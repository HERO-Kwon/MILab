...
for n_splits in list_cv:
    ...
    for rs_num in list_rs:
        ...
        for i,[train_index,test_index] in enumerate(skf.split(prep_xs,prep_ys)):
            ...
            #TER's
            for order in np.arange(1,5,1):
            #for m in range(4):
                m=1
                print('TER_order:' + str(order))
                
                #RM
                X_rm = RMmodel(X,order)
                Xval_rm = RMmodel(Xval,order)
                
                # TER Dual
                m_terd = TERdual([X_rm,Y,Xval_rm,Yval],0.5,0.5,m)
