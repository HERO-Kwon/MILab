...
with gzip.open(path_fc+'arr_score'+'.pickle.gz','rb') as f:
...
def make_validation(x,y):
    ...
    arr_xs = np.array(list_xs).reshape([-1,2*n_features])
    arr_ys = np.array(list_ys).reshape([-1,1])
...
X,Y = make_validation(fc1,fc1lab)
Xval,Yval = make_validation(fc2,fc2lab)
...
list_rs = [10]
    ...
    for rs_num in list_rs:
        ...
        #TER's
        for order in np.arange(1,3,1):
        #for m in range(4):
            m=1
            print('TER_order:' + str(order))

            #RM
            X = RMmodel(X,order)
            Xval = RMmodel(Xval,order)
            ...
            res_ser = pd.Series({'name':m_terd.model_name,'splits':n_splits,'splits_number':i,
                                'random_state':rs_num,'eer':eer_terd,'param0':order,'tr_time':m_terd.train_time}) 
            m_results = m_results.append(res_ser,ignore_index=True)


        #SVM Linear
        m_svml = SVMlinear([X,Y,Xval,Yval])
        dist_svml = m_svml.val_dist(m_svml.train(c=1.0)) 
        eer_svml = m_svml.eer_graphs(dist_svml.truth,dist_svml.score,0)

        #gather results
        res_ser = pd.Series({'name':m_svml.model_name,'splits':n_splits,'splits_number':i,
                            'random_state':rs_num,'eer':eer_svml,'param0':0,'tr_time':m_svml.train_time}) 
        m_results = m_results.append(res_ser,ignore_index=True)  

        #SVM RBF
        num_features = X.shape[1]
        for g in [0.1/num_features, 1/num_features, 10/num_features]:
            print('RBF:' + str(g))

            m_svmr = SVMrbf([X,Y,Xval,Yval])
            dist_svmr = m_svmr.val_dist(m_svmr.train(c=1.0,gamma=g)) 
            eer_svmr = m_svmr.eer_graphs(dist_svmr.truth,dist_svmr.score,0)

            #gather results
            res_ser = pd.Series({'name':m_svmr.model_name,'splits':n_splits,'splits_number':i,
                                'random_state':rs_num,'eer':eer_svmr,'param0':g,'tr_time':m_svmr.train_time}) 
            m_results = m_results.append(res_ser,ignore_index=True)  
        '''
        #SVM Poly
        for d in np.arange(1,3,1):
            print('Poly:' + str(d))

            m_svmp = SVMpoly([X,Y,Xval,Yval])
            dist_svmp = m_svmp.val_dist(m_svmp.train(c=1.0,degree=d)) 
            eer_svmp = m_svmp.eer_graphs(dist_svmp.truth,dist_svmp.score,0)            

            #gather results
            res_ser = pd.Series({'name':m_svmp.model_name,'splits':n_splits,'splits_number':i,
                                'random_state':rs_num,'eer':eer_svmp,'param0':d,'tr_time':m_svmp.train_time}) 
            m_results = m_results.append(res_ser,ignore_index=True)  
        '''
        m_results.to_csv('m_v4_results_11.csv')