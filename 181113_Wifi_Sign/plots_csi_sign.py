...
def plot_animated(path,exp_id,arr_scaled):
    ...
    def animate(i):
        ...
        line100.set_ydata(np.cos(np.angle(arr_scaled[i,:,0,0])))  # update the data.
        line101.set_ydata(np.cos(np.angle(arr_scaled[i,:,0,1])))
        line102.set_ydata(np.cos(np.angle(arr_scaled[i,:,0,2])))
        line110.set_ydata(np.cos(np.angle(arr_scaled[i,:,1,0])))  # update the data.
        line111.set_ydata(np.cos(np.angle(arr_scaled[i,:,1,1])))
        line112.set_ydata(np.cos(np.angle(arr_scaled[i,:,1,2])))    
        return [line000,line001,line002,line010,line011,line012,line100,line101,line102,line110,line111,line112],
    ...
    ani.save(path+exp_id+'_ani.mp4', writer = FFwriter)

    return fig

def heatmap_array(path,size,exp_id,arr,vmax,vmin):
    hmap = plt.subplots(figsize=(size[0], size[1]))
    hmap = sns.heatmap(arr,vmax=vmax,vmin=vmin)
    fig = hmap.get_figure()
    fig.savefig(path+'hmap_'+exp_id+'.png')
    return fig