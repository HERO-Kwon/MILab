...
#folder_list = folder_list[0:3304]
#folder_list = folder_list[3304:5782]
folder_list = folder_list[5782:len(folder_list)]

infiles = set([file.replace(".pickle.gz","") for file in os.listdir(save_path) if not "subc" in file])
tofiles = list(set(folder_list) - infiles)

df_subc = pd.DataFrame()
dict_scaled = {}
for exp_id in tofiles:
