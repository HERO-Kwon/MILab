...
path_mit_image = '/home/herokwon/mount_data/Data/Wi-Fi_mit_angle/'
...
subp_arr = np.arange(0,30000,30000/10,dtype=np.int32) # 19 subprocess len 27375
df_info = df_info.iloc[subp_arr[0]:subp_arr[1]]
...
ax = sns.heatmap(sig_mean[0,:2*th_range,:2*si_range])#,vmin=np.median(sig_mat))

#ax = sns.heatmap(sig_mat[0,0,:2*th_range,:2*si_range])#,vmin=np.median(sig_mat))
