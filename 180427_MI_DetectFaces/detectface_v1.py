...
file_list = os.listdir(file_path)[0:14]
file_list = [s for s in file_list if s.endswith(".bmp")]
...
for file in file_list:
    ...
    if len(faces) != 0:
        left_eye = faces[0].landmarks[0].position.x, faces[0].landmarks[0].position.y, faces[0].landmarks[0].position.z
        right_eye = faces[0].landmarks[1].position.x, faces[0].landmarks[1].position.y, faces[0].landmarks[1].position.z

        box = [(vertex.x, vertex.y) for vertex in faces[0].bounding_poly.vertices]
